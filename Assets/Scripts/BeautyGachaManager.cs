...
public class BeautyGachaManager : MonoBehaviour
{
    ...
    void UpdateStatusDisplay()
    {
        if (statusText != null)
        {
            ...
            string familyInfo = currentFamilyWealth != null ? $"🏠 家柄: {currentFamilyWealth.name} (片{currentFamilyWealth.beautyPoints})" : "🏠 家柄: 特になし (片 0)";
            string personalityInfo = currentPersonality != null ? $"😊 性格: {currentPersonality.name} (片{currentPersonality.beautyPoints})" : "😊 性格: 特になし (片 0)";
